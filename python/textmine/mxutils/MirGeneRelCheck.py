...
class MirGeneRelCheck:
    ...
    def __init__(self, nlp_ent=None):
        ...
        self.surGeneContexts = [
            ...
            (re.compile('(%s){e<=1}' % "family member"), 16),
            ...
            #website
            (re.compile('(%s){e<=1}' % "family"), 16),
            (re.compile('(%s){e<=1}' % "assay"), 10),
            (re.compile('(%s){e<=1}' % "disease"), 10),
            (re.compile('(%s){e<=1}' % "mutations"), 16),
            (re.compile('(%s){e<=1}' % "stimulation"), 16),
            (re.compile('(%s){e<=1}' % "Figure"), 10),
            (re.compile('(%s){e<=1}' % "Table"), 10),
            (re.compile('(%s){e<=0}' % "Fig."), 10),
            (re.compile('(%s){e<=0}' % "Tab."), 10),
            
            (re.compile('(%s){e<=1}' % "samples"), 16),
            (re.compile('(%s){e<=1}' % "tissue"), 10),
            (re.compile('(%s){e<=1}' % "progression"), 12),
            (re.compile('(%s){e<=1}' % "pathogenesis"), 16),
            (re.compile('(%s){e<=1}' % "subtype"), 10),
            (re.compile('(%s){e<=1}' % "tumor"), 10),
            (re.compile('(%s){e<=1}' % "group"), 10),

            
            (re.compile('(%s){e<=0}' % "-induced"), 10),
            #(re.compile('(%s){e<=0}' % "-mediated"), 10),
            (re.compile('(%s){e<=1}' % "mouse model"), 16),
        ]
        ...
        self.surPreGeneContexts = [
            ...
            (re.compile('(%s){e<=1}' % "downstream .* target of"), 35),
            ...
            # website cancers
            (re.compile('(%s){e<=1}' % "biomarker for"), 15),
            (re.compile('(%s){e<=1}' % "component in"), 15),
            (re.compile('(%s){e<=1}' % "recurrent"), 15),
            (re.compile('(%s){e<=1}' % "recurrence"), 15),
            (re.compile('(%s){e<=1}' % "classic"), 15),
            (re.compile('(%s){e<=1}' % "invasive"), 15),
            (re.compile('(%s){e<=1}' % "aggressive"), 15),
            #(re.compile('(%s){e<=0}' % "variant of"), 15), ??
            (re.compile('(%s){e<=1}' % "stages of"), 15),
            (re.compile('(%s){e<=1}' % "prognosis in"), 15),
            (re.compile('(%s){e<=1}' % "prognosis of"), 15),
            
            (re.compile('(%s){e<=1}' % "factors in"), 15),            
            (re.compile('(%s){e<=1}' % "up-regulated in"), 18),
            (re.compile('(%s){e<=1}' % "over-expressed in"), 18),
            (re.compile('(%s){e<=1}' % "down-regulated in"), 20),
            (re.compile('(%s){e<=1}' % "under-expressed in"), 20),
            (re.compile('(%s){e<=0}' % "increased in"), 18),
            (re.compile('(%s){e<=0}' % "decreased in"), 18),
            
            (re.compile('(%s){e<=1}' % "carcinogenesis of"), 20),
        ]
    ...
    def check_entity(self, doc, mirword, geneword, verbose):
        ...
        if self.nlp_ent != None:
            ...
            ignoreEntities = set(['CELL', "CANCER"]).intersection(ents)
            if len(ignoreEntities) > 0:
                if verbose:
                    print("Gene reject:", ents, "due to", ignoreEntities)                
                return False
    ...
    def checkSDP(self, doc, ent1, ent2, verbose):
        ...
        # [(PD-L1, 'NOUN', 'compound', False), (expression, 'NOUN', 'dobj', False), (promoted, 'VERB', 'ROOT', False), (sponging, 'ADJ', 'advcl', False), (miR-34a, 'NOUN', 'dobj', False)]
        verbIdx = None
        dObjIdx = None
        for idx in range(0, len(sdpRes)-1):

            if sdpRes[idx][1] in ["VERB"]:
                if verbIdx is None:
                    verbIdx = idx
            if sdpRes[idx][1] in ["NOUN"] and sdpRes[idx][2] == "dobj":
                if dObjIdx is None:
                    dObjIdx = idx
                    
        if not None in [verbIdx, dObjIdx]:
            if dObjIdx < verbIdx:
                if verbose:
                    print("SDP DOBJ RULE")
                #sdpPass = False
